...
// Keyed sparkline — remounts on range change so Chart.js always gets a fresh canvas
function Sparkline({ closes, color, rangeKey }) {
  ...
  useEffect(() => {
    ...
    chartRef.current = new Chart(canvasRef.current, {
      ...
      options: {
        ...
        scales: {
          x: { display: false },
          y: { display: false, min: min * 0.995, max: max * 1.005 },
        },
      },
    })
    return () => { if (chartRef.current) { chartRef.current.destroy(); chartRef.current = null } }
  }, [rangeKey]) // only re-run when range changes, not on every render
...
export default function TickerCard({ allRangeResults, sym, isSP500=false, isFav=false, onToggleFav, onClick }) {
  ...
  // Price and today's change always come from meta (same regardless of range)
  ...
  // Closes for the selected range's sparkline
  const rawCloses = result?.indicators?.quote?.[0]?.close ?? []
  const closes    = rawCloses.filter(v => v != null)

  // Closes for 52w bar — always 1Y
  ...
  return (
    ...
    >
      ...
      <div onClick={e => e.stopPropagation()} style={{ marginTop:2, marginBottom:2 }}>
        <RangeToggle
          active={activeRange}
          onChange={r => setActiveRange(r.label)}
          color={isSP500 ? '#4a8fd4' : '#888892'}
        />
      </div>
      ...
      {!result ? (
        ...
        <>
          ...
          <div style={{ display:'flex', gap:8, alignItems:'baseline', flexWrap:'wrap' }}>
            <div style={{ fontSize:11, fontWeight:500, color: isUp ? 'var(--up)' : 'var(--dn)' }}>
              {isUp ? '+' : ''}{pct?.toFixed(2)}% today
            </div>
            {/* Period return — only show when different from 1D */}
          ...
          {/* Sparkline — keyed by sym+range so it fully remounts on switch */}
          {closes.length >= 2 && (
            <div style={{ marginTop:4 }}>
              <Sparkline
                closes={closes}
                color={sparkColor}
                rangeKey={`${sym}-${activeRange}`}
              />
            </div>
          )}

          {/* 52w range bar — always from 1Y */}
          {!isSP500 && closes1Y.length > 0 && (
            <WeekRange closes={closes1Y} price={price} />
          )}

          {!isSP500 && (
            <div style={{ fontSize:9, color:'var(--text-muted)', textAlign:'center', opacity:0.5, marginTop:3 }}>
              click to expand
            </div>
          )}
        </>
