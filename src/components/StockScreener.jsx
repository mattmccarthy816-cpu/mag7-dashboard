...
const CHANGE_FILTERS = [
  ...
  { label:'↑↑ +2%+',    fn: s => (s.changePercent??0) >= 2  },
  { label:'↓↓ -2%+',    fn: s => (s.changePercent??0) <= -2 },
]
const SORT_OPTIONS = [
...
export default function StockScreener({ onTickerClick }) {
    ...
    for (let i = 0; i < chunks.length; i += PARALLEL) {
      ...
      const batchResults = await Promise.all(
        batch.map(async (chunk) => {
          try {
            const url = '/api/screen?symbols=' + chunk.join(',')
            const res = await fetch(url)
            if (!res.ok) { console.warn('screen', res.status); return [] }
            const data = await res.json()
            if (!Array.isArray(data)) { console.warn('screen non-array:', data); return [] }
            return data
          } catch (e) {
            console.warn('screen error:', e.message)
            return []
          }
        })
      )
      ...
      setProgress(Math.min(99, Math.round(((i + PARALLEL) / chunks.length) * 100)))
    }
  ...
  const filtered = useMemo(() => {
    ...
    return stocks
      .filter(s => {
        ...
        if (!CHANGE_FILTERS[changeF].fn(s)) return false
        return true
      })
      .sort(SORT_OPTIONS[sortBy].fn)
  }, [stocks, mcapTier, peRange, priceRange, changeF, sortBy, sector, searchQ])

  const activeCount = [mcapTier,peRange,priceRange,changeF].filter(v=>v!==0).length + (sector!=='All'?1:0) + (searchQ?1:0)

  const reset = () => { setMcapTier(0); setPeRange(0); setPriceRange(0); setChangeF(0); setSortBy(0); setSector('All'); setSearchQ('') }

  const panelStyle = { background:'var(--bg-card)', border:'0.5px solid var(--border)', borderRadius:'var(--radius)', padding:'16px' }
  ...
  if (loadState === 'idle') return (
    <div style={panelStyle}>
      ...
      <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:14, padding:'24px 0' }}>
        <div style={{ fontSize:13, color:'var(--text-secondary)', textAlign:'center', lineHeight:1.7, maxWidth:420 }}>
          Filter ~{SP500_SYMBOLS.length} S&P 500 stocks by market cap, P/E ratio, price, and daily movement.
          Data loads once and all filters update instantly.
        </div>
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
      ...
      {/* Filters panel — show once any data arrives */}
      {stocks.length > 0 && (
        <div style={panelStyle}>
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:12, flexWrap:'wrap', gap:8 }}>
            <div style={{ fontSize:12, fontWeight:600, color:'var(--text-secondary)', letterSpacing:'0.05em', textTransform:'uppercase' }}>
              Stock Screener
              {loadState==='done' && <span style={{ fontWeight:400, color:'var(--text-muted)', marginLeft:8 }}>{filtered.length} of {stocks.length} stocks</span>}
            </div>
            {activeCount > 0 && (
              <button onClick={reset} style={{ fontSize:11, padding:'5px 12px', background:'rgba(224,80,80,0.1)', border:'0.5px solid rgba(224,80,80,0.3)', borderRadius:7, color:'#e05050', cursor:'pointer' }}>
          ...
          <div style={{ display:'flex', gap:8, marginBottom:12, flexWrap:'wrap' }}>
            <input
              ...
              style={{ flex:1, minWidth:150, fontSize:12, padding:'7px 11px', background:'var(--bg-secondary)', border:'0.5px solid var(--border-strong)', borderRadius:7, color:'var(--text-primary)', outline:'none' }}
            />
          ...
          {/* Filter rows */}
          {[
            { label:'Market Cap', items:MCAP_TIERS, active:mcapTier, set:setMcapTier, color:'#4a8fd4' },
            { label:'P/E Ratio',  items:PE_RANGES,   active:peRange,  set:setPeRange,  color:'#e8a835' },
            { label:'Price',      items:PRICE_RANGES, active:priceRange, set:setPriceRange, color:'#9b7de0' },
            { label:"Today's Move", items:CHANGE_FILTERS, active:changeF, set:setChangeF, color:'#1fb87a' },
          ].map(({ label, items, active, set, color }) => (
            <div key={label} style={{ marginBottom:9 }}>
              <div style={{ fontSize:10, fontWeight:600, color:'var(--text-muted)', marginBottom:4, textTransform:'uppercase', letterSpacing:'0.05em' }}>{label}</div>
            ...
            <div style={{ overflowX:'auto' }}>
              <table style={{ width:'100%', borderCollapse:'collapse', fontSize:12 }}>
                ...
                <tbody>
                  {filtered.slice(0,100).map((s,i) => {
                    ...
                    return (
                      <tr key={s.sym} onClick={() => onTickerClick?.(s.sym)}
                      ...
                      >
                        ...
                        <td style={{ padding:'7px 8px', color:up?'#1fb87a':'#e05050', whiteSpace:'nowrap', fontVariantNumeric:'tabular-nums' }}>{fmtPct(s.changePercent)}</td>
              ...
              {filtered.length > 100 && (
                <div style={{ fontSize:11, color:'var(--text-muted)', padding:'10px 8px' }}>
                  Showing top 100 of {filtered.length} · refine filters to narrow results
                </div>
              )}
            </div>
          )}
