...
export default function SectorHeatmap({ sectorEtfResults, spyResult, activeSector, onSectorClick }) {
  const sectorCells = SECTORS.map(s => {
    const closes = extractCloses(sectorEtfResults?.[s.id])
  ...
  const sp500Cell = {
    id:'sp500', label:'S&P 500', color:'#4a8fd4',
    pct1m: spN > 21 ? pctChange(spyCloses[spN-1], spyCloses[spN-22]) : null,
    pct1y: spN > 1  ? pctChange(spyCloses[spN-1], spyCloses[0]) : null,
    sector: null,
  }

  // Sort sectors by 1m perf, then pin S&P 500 last
  const sorted = [...sectorCells].sort((a,b) => (b.pct1m??-99) - (a.pct1m??-99))
  const allCells = [...sorted, sp500Cell]
    ...
    <Panel title="Sector Rotation" badge="1 month · click to switch">
      <div style={{ display:'grid', gridTemplateColumns:'repeat(3, minmax(0,1fr))', gap:5 }}>
        {allCells.map(cell => {
          ...
          return (
            <div key={cell.id} onClick={() => cell.sector && onSectorClick(cell.sector)}
              style={{
                background: bg,
              ...
              onMouseEnter={e=>{ if(cell.sector) e.currentTarget.style.opacity='0.75' }}
      ...
      <div style={{ fontSize:10, color:'var(--text-muted)', marginTop:8 }}>
        Sorted by 1-month return · S&P 500 pinned for comparison
      </div>
    </Panel>
  )
}
